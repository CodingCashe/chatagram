...
// export async function deleteScheduledContent(clerkId: string, instagramPostId: string) {
//   try {
//     const user = await client.user.findUnique({
//       where: { clerkId },
//       include: { integrations: { where: { name: "INSTAGRAM" } } },
...
//       throw new Error("Instagram ID or token not found for this integration")
//     }

//     const instagramApi = new InstagramAPI(integration.token)
//     await instagramApi.deleteScheduledContent(integration.instagramId, instagramPostId)
...
// export async function updateScheduledContent(clerkId: string, instagramPostId: string, contentData: any) {
//   try {
//     const user = await client.user.findUnique({
//       where: { clerkId },
//       include: { integrations: { where: { name: "INSTAGRAM" } } },
...
//     const instagramApi = new InstagramAPI(integration.token)
//     const updatedContent = await instagramApi.updateScheduledContent(
...
//     const instagramApi = new InstagramAPI(integration.token)
//     const refreshedToken = await refreshToken(integration.refreshToken)
//     if (refreshedToken) {
//       await client.integrations.update({
//         where: { id: integration.id },
//         data: { token: refreshedToken.access_token, refreshToken: refreshedToken.refresh_token },
//       })
//       integration.token = refreshedToken.access_token
//       integration.refreshToken = refreshedToken.refresh_token
//     }

//     const instagramData = await instagramApi.getBusinessAccountInfo(integration.instagramId)
//     console.log(`[refreshInstagramData] Instagram data fetched:`, instagramData)

//     await client.user.update({
//       where: { id: user.id },
//       data: {
//         instagramBusinessAccountId: instagramData.id,
