...
export default function PricingSection() {
  ...
  const getRecommendation = () => {
    ...
    // Simple recommendation logic
    let recommendedPlanId = "free";
  
    if (teamSize === "large" || useCase === "enterprise") {
      recommendedPlanId = "enterprise";
    } else if ((teamSize === "small" && (useCase === "business" || importantFeature === "analytics")) || 
        (importantFeature === "security" && useCase !== "personal")) {
      recommendedPlanId = "pro";
    } else if (teamSize === "small" && useCase === "development") {
      recommendedPlanId = "starter";
    } else if (teamSize === "just-me" && (useCase === "personal" || importantFeature === "integrations")) {
      if (importantFeature === "support") {
        recommendedPlanId = "starter";
      } else {
    ...
    // Set the current slide to the recommended plan
    const planIndex = pricingPlans.findIndex(plan => plan.id === recommendedPlanId);
    if (planIndex !== -1) {
  ...
  return (
    <div className="space-y-16">
      ...
      <div className="mt-10 relative max-w-4xl mx-auto px-4">
        {/* Navigation Arrows */}
        <div className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 z-20 hidden md:block">
          <Button
            variant="outline"
            size="icon"
            className="rounded-full bg-gray-800/70 border-gray-700 text-white hover:bg-gray-700 h-10 w-10"
            onClick={() => setCurrentSlide((prev) => (prev === 0 ? pricingPlans.length - 1 : prev - 1))}
          >
        ...
        <div className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 z-20 hidden md:block">
          <Button
            ...
            size="icon"
            className="rounded-full bg-gray-800/70 border-gray-700 text-white hover:bg-gray-700 h-10 w-10"
            onClick={() => setCurrentSlide((prev) => (prev === pricingPlans.length - 1 ? 0 : prev + 1))}
          >
        ...
        <div className="overflow-hidden">
          <motion.div
            ...
            style={{ width: `${pricingPlans.length * 100}%` }}
          >
            {pricingPlans.map((plan, index) => (
              <div
                key={plan.id}
                className="w-full md:w-1/4 px-2 md:px-4"
                style={{ width: `${100 / pricingPlans.length}%` }}
                onMouseEnter={() => setHoveredPlan(plan.id)}
                onMouseLeave={() => setHoveredPlan(null)}
              >
                <div className="relative">
                  ...
                  <Card
                    id={`plan-${plan.id}`}
                    className={`
              relative overflow-hidden border-gray-800 bg-gray-900/80 backdrop-blur-sm h-full
              ${plan.popular ? "border-gray-700 mt-4" : "border-gray-800"}
              ${hoveredPlan === plan.id ? "transform-gpu -translate-y-1 transition-transform" : "transition-transform"}
              ${plan.id === activePlan ? "ring-2 ring-green-500/30" : ""}
            `}
          ...
          {pricingPlans.map((_, index) => (
            <button
              ...
              className={`h-2 rounded-full transition-all ${
                currentSlide === index ? "w-6 bg-white" : "w-2 bg-gray-600"
              }`}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Button 
                      ...
                      onClick={() => {
                        ...
                        const planIndex = pricingPlans.findIndex(plan => plan.id === recommendedPlan);
                        if (planIndex !== -1) {
                          setCurrentSlide(planIndex);
