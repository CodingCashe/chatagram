...
import { useState, useEffect, useRef, useCallback } from "react"
import { motion, AnimatePresence } from "framer-motion"
import { Send, ArrowLeft, Smile, Paperclip, Mic, Trash2, Search, X } from "lucide-react"
import { ScrollArea } from "@/components/ui/scroll-area"
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar"
import { Textarea } from "@/components/ui/textarea"
import { Button } from "@/components/ui/button"
...
const AutomationChats: React.FC<AutomationChatsProps> = ({ automationId }) => {
  ...
  const getAvatarUrl = (userId: string | undefined) => {
    return `https://api.dicebear.com/6.x/initials/svg?seed=${userId || "default"}`
  }
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const userId = selectedConversation.userId ? `${pageId}_${selectedConversation.userId}` : `${pageId}_unknown`
      const result = await sendMessage(newMessage, userId, pageId, automationId, token, businessVariables)
  ...
  const getFancyName = (userId: string | undefined) => {
  ...
  const filteredConversations = conversations.filter((conversation) =>
    getFancyName(conversation.userId).toLowerCase().includes(searchTerm.toLowerCase()),
  )
        ...
        <>
          {selectedConversation ? (
            <>
              ...
              <ScrollArea className="flex-grow overflow-y-auto" ref={scrollRef}>
                <div className="p-4 space-y-4">
                  <AnimatePresence initial={false}>
                    {selectedConversation.messages.map((message, index) => (
                      <MessageBubble
                        key={message.id}
                        message={message}
                        isLastMessage={index === selectedConversation.messages.length - 1}
                        botName={BOT_NAME}
            ...
            <>
              ...
              <ScrollArea className={selectedConversation ? "flex-grow" : "max-h-[calc(100vh-12rem)]"}>
                {!token ? (
                  <div className="p-4 space-y-4">
                    <ExampleConversations onSelectConversation={handleSelectConversation} className="px-4" />
                    <div className="text-center">
                  ...
                  <AnimatePresence>
                    {filteredConversations.slice(0, displayedConversations).map((conversation, index) => (
                      <motion.div
                        key={conversation.id}
                      ...
                      >
                        <div className="flex-grow" onClick={() => handleSelectConversation(conversation)}>
                          <Avatar className="w-10 h-10 relative border-2 border-primary">
                            <AvatarImage src={getAvatarUrl(conversation.userId)} />
                            <AvatarFallback>{getFancyName(conversation.userId)?.slice(0, 2) || "UC"}</AvatarFallback>
                          </Avatar>
                          <div className="ml-3 flex-grow overflow-hidden pr-4">
                            <p className="font-medium text-sm text-foreground">{getFancyName(conversation.userId)}</p>
                            <p className="text-xs text-muted-foreground truncate">
                ...
                {displayedConversations < filteredConversations.length && (
                  <div className="p-4">
                    <Button
                      onClick={() =>
                        setDisplayedConversations((prev) => Math.min(prev + 4, filteredConversations.length))
                      }
                      variant="outline"
                      className="w-full"
                    >
