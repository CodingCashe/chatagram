...
//   return (
//     <div
//       className={`flex flex-col bg-gradient-to-br from-[#2A2A2A] via-[#252525] to-[#1D1D1D] text-foreground border border-primary/10 rounded-lg overflow-hidden ${
...
//                         >
//                           <Avatar className="w-10 h-10 relative border-2 border-primary flex-shrink-0">
//                             <AvatarImage src={getAvatarUrl(conversation.id)} />
//                             <AvatarFallback>{getFancyName(conversation.id).slice(0, 2)}</AvatarFallback>
//                             {unreadChats.has(conversation.id) && (
//                               <span className="absolute top-0 right-0 block h-3 w-3 rounded-full bg-primary transform translate-x-1/2 -translate-y-1/2"></span>
...
const AutomationChats: React.FC<AutomationChatsProps> = ({ automationId }) => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // if (result.success && result.userMessage) {
      //   const userMessage: Message = {
      //     id: result.userMessage.id || tempMessageId,
      //     role: "user",
      //     content: result.userMessage.content,
      //     senderId: selectedConversation.userId,
      //     receiverId: pageId,
                    ...
                    >
                      ...
                      >
                        ...
                        <div className="flex justify-between items-center mt-1 text-xs text-gray-300">
                          ...
                          {message.role === "user" && (
                            <div
                              className={`flex items-center ${message.status === "sent" ? "text-green-400" : "text-red-400"}`}
                            >
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button
                            ...
                            onClick={handleSendMessage}
                            className={cn(
                              "bg-primary hover:bg-green text-primary-foreground h-7 w-7 rounded-full flex-shrink-0",
