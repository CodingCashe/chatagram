...
import { Popover, PopoverContent, PopoverTrigger } from "@/components/ui/popover"
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/components/ui/tooltip"
import type { Conversation as DashboardConversation, Message } from "@/types/dashboard"
import data from "@emoji-mart/data"
import Picker from "@emoji-mart/react"
import ExampleConversations from "./exampleConvo"
import DeleteConfirmationModal from "./confirmDelete"
...
type ConversationWithExtra = DashboardConversation & {
  chatId: string
  pageId: string
  userId: string
  Automation: null
...
const ShimmeringBorder = ({ children }: { children: React.ReactNode }) => {
  return <div className={`${gradientBorder} p-[2px] rounded-lg`}>{children}</div>
}

const AutomationChats: React.FC<AutomationChatsProps> = ({ automationId }) => {
  const [conversations, setConversations] = useState<ConversationWithExtra[]>([])
  const [selectedConversation, setSelectedConversation] = useState<ConversationWithExtra | null>(null)
  const [newMessage, setNewMessage] = useState("")
  const [isTyping, setIsTyping] = useState(false)
  const [isLoading, setIsLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  ...
  const [displayedConversations, setDisplayedConversations] = useState(4)
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false)
  const [conversationToDelete, setConversationToDelete] = useState<ConversationWithExtra | null>(null)
  const [aiSuggestion, setAiSuggestion] = useState<string | null>(null)
  const scrollRef = useRef<HTMLDivElement>(null)
  ...
  }, [scrollRef]) // Removed conversations from dependencies
  ...
  const handleSelectConversation = (conversation: ConversationWithExtra) => {
    setSelectedConversation(conversation)
    setUnreadChats((prev) => {
      const newUnreadChats = new Set(prev)
      newUnreadChats.delete(conversation.id)
  ...
  const handleDeleteConversation = (conversation: ConversationWithExtra) => {
    setConversationToDelete(conversation)
    setIsDeleteModalOpen(true)
  }
  ...
  return (
    <ShimmeringBorder>
      <div
        className={`flex flex-col ${fancyBackground} text-foreground rounded-lg overflow-hidden ${
          selectedConversation ? "h-[calc(100vh-4rem)]" : "h-[calc(100vh-4rem)]"
      ...
      >
        {isLoading ? (
          ...
          <>
            {selectedConversation ? (
              <>
                <div className="p-2 md:p-4 bg-background border-b border-primary/10 flex items-center">
                  <Button
                    variant="ghost"
                    className="mr-2 md:mr-4 p-1 md:p-2"
                  ...
                  <div className="ml-2 md:ml-3 flex-grow">
                    <h4 className="font-medium text-base md:text-lg">{getFancyName(selectedConversation.id)}</h4>
                    {selectedConversation && selectedConversation.messages.length > 0 && (
                  ...
                  <div className="p-2 md:p-4 space-y-2 md:space-y-4">
                    {selectedConversation.messages.map((message, index) => (
                      <motion.div
                        ...
                        animate={{ opacity: 1, y: 0, scale: 1 }}
                        transition={{ duration: 0.3, delay: index * 0.1 }}
                        className={`flex items-end mb-2 md:mb-4 ${message.role === "assistant" ? "justify-end" : "justify-start"}`}
                      >
                        {message.role === "assistant" ? (
                          <Avatar className="w-6 h-6 md:w-8 md:h-8 mr-2 border-2 border-primary">
                            <AvatarImage src={BOT_AVATAR} />
                            <AvatarFallback>{BOT_NAME.slice(0, 2)}</AvatarFallback>
                          </Avatar>
                        ) : (
                          <Avatar className="w-6 h-6 md:w-8 md:h-8 ml-2 order-last border-2 border-primary">
                            <AvatarImage src={`https://i.pravatar.cc/150?u=${message.senderId}`} />
                            <AvatarFallback>{getFancyName("123456789").slice(0, 2)}</AvatarFallback>
                          </Avatar>
                        )}
                        <div
                          className={cn(
                            "max-w-[75%] p-2 md:p-3 rounded-3xl text-xs md:text-sm relative",
                            message.role === "assistant"
                              ? "bg-gradient-to-br from-blue-400/30 to-blue-600/30 border-2 border-blue-500/50 text-white"
                              : "bg-gradient-to-br from-purple-400/30 to-purple-600/30 border-2 border-purple-500/50 text-white",
                          )}
                ...
                <div className="p-2 md:p-4 bg-background border-t border-primary/10">
                  <div className="flex space-x-2 mb-2 overflow-x-auto pb-2">
                    {["Hello!", "How can I help?", "Thank you!", "I'll get back to you soon."].map(
                      (response, index) => (
                        <motion.button
                          ...
                          whileHover={{ scale: 1.05 }}
                          whileTap={{ scale: 0.95 }}
                          className="bg-primary/20 text-primary px-2 md:px-3 py-1 rounded-full text-xs md:text-sm whitespace-nowrap"
                          onClick={() => setNewMessage(response)}
                        >
                          {response}
                        </motion.button>
                  ...
                  {isTyping && (
                    <motion.div
                      ...
                      className="flex items-center space-x-2 p-2 bg-muted rounded-lg mb-2"
                    >
                      <span className="text-xs md:text-sm text-muted-foreground">AiAssist is typing</span>
                      <motion.div
                        animate={{
                          scale: [1, 1.2, 1],
                          transition: { repeat: Number.POSITIVE_INFINITY, duration: 1 },
                  ...
                  <div className="flex items-center space-x-1 md:space-x-2 relative">
                    ...
                    <div className="flex-grow relative">
                      <Textarea
                        ...
                        onChange={(e) => setNewMessage(e.target.value)}
                        onKeyPress={(e) => e.key === "Enter" && !e.shiftKey && handleSendMessage()}
                        className="flex-grow text-xs md:text-sm bg-muted border-primary/20 text-foreground placeholder-muted-foreground min-h-[36px] max-h-[96px] py-1 md:py-2 px-1 md:px-2 rounded-lg resize-none overflow-hidden"
                        style={{ height: "36px", transition: "height 0.1s ease" }}
                        onInput={(e) => {
                          const target = e.target as HTMLTextAreaElement
                          target.style.height = "36px"
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <motion.button
                              ...
                              whileTap={{ scale: 0.9 }}
                              onClick={handleSendMessage}
                              className="bg-primary hover:bg-green text-primary-foreground h-6 w-6 md:h-7 md:w-7 rounded-full flex-shrink-0 flex items-center justify-center"
                            >
                  ...
                  {aiSuggestion && (
                    ...
                    >
                      ...
                      <p className="text-xs md:text-sm text-blue-500">{aiSuggestion}</p>
                      <button
                        className="text-xs text-blue-700 hover:underline"
                        onClick={() => setNewMessage(aiSuggestion)}
                      >
              ...
              <>
                <h3 className="text-base md:text-lg font-semibold p-2 md:p-4 bg-background flex justify-between items-center">
                  <span>Recent Chats</span>
                  {totalUnreadMessages > 0 && (
                    <span className="bg-red-500 text-primary-foreground text-xs font-bold px-2 py-1 rounded-full">
                      {totalUnreadMessages}
                ...
                <ScrollArea className={selectedConversation ? "flex-grow" : "h-[calc(100vh-8rem)]"}>
                  {!token ? (
                    <div className="p-4 space-y-4">
                      <ExampleConversations onSelectConversation={handleSelectConversation} className="px-4" />
                      <div className="text-center">
                    ...
                    <>
                      {conversations.slice(0, displayedConversations).map((conversation) => (
                        <motion.div
                          ...
                          animate={{ opacity: 1, y: 0 }}
                          transition={{ duration: 0.2 }}
                          className="flex items-center p-2 md:p-4 hover:bg-muted cursor-pointer transition-colors duration-200"
                        >
                          <div className="flex-grow" onClick={() => handleSelectConversation(conversation)}>
                            <Avatar className="w-8 h-8 md:w-10 md:h-10 relative border-2 border-primary">
                              <AvatarImage src={getAvatarUrl(conversation.id)} />
                              <AvatarFallback>{getFancyName(conversation.id).slice(0, 2)}</AvatarFallback>
                              {unreadChats.has(conversation.id) && (
                                <span className="absolute top-0 right-0 block h-2 w-2 md:h-3 md:w-3 rounded-full bg-primary transform translate-x-1/2 -translate-y-1/2"></span>
                              )}
                            </Avatar>
                            <div className="ml-2 md:ml-3 flex-grow overflow-hidden pr-4">
