...
// Enhance the OfflineNotification component to make it more useful:

// Replace the OfflineNotification component with this improved version:
// Offline notification component
const OfflineNotification = ({ onRetry }: { onRetry: () => void }) => {
  return (
    <motion.div
      initial={{ opacity: 0, y: -20 }}
      animate={{ opacity: 1, y: 0 }}
      ...
      <div className="flex items-center space-x-2">
        ...
        <div className="flex space-x-2">
          ...
          <Button
            variant="outline"
            size="sm"
            className="h-7 text-xs bg-yellow-700 hover:bg-yellow-800 border-yellow-500"
...
// Add a retry button to the FancyErrorMessage component to allow manual retry:

// Replace the FancyErrorMessage component with this improved version:
const FancyErrorMessage: React.FC<{ message: string }> = ({ message }) => {
  const [isLoading, setIsLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full p-4 text-center">
      ...
      <div className="flex flex-col items-center gap-4">
        ...
        {message.includes("sample") || message.includes("Unable to load") ? (
          <Button
            variant="outline"
            onClick={() => {
              setIsLoading(true)
...
const AutomationChats: React.FC<AutomationChatsProps> = ({ automationId }) => {
  ...
  const fetchChats = useCallback(
    async (preserveReadStatus = false) => {
      ...
      if (!preserveReadStatus) {
        // Shorter timeout (8 seconds) to avoid long waits
        fetchTimeout = setTimeout(() => {
          ...
          if (fetchAttemptsRef.current <= 1) {
            setError("Request timed out. Trying with sample data instead...")

            // Use mock data after timeout
            ...
            // Update error message after showing data
            setTimeout(() => {
              setError("Unable to load live data. Showing sample conversations instead.")
            }, 1000)
          } else {
            setError("Connection issues detected. Showing sample data instead.")
          }
      ...
      try {
        ...
        // Use mock data after 2 attempts instead of waiting for 5 attempts
        if (fetchAttemptsRef.current >= 2) {
          console.log("Using mock data after multiple failed attempts")
          const mockConversations: Conversation[] = [
            {
              id: "mock_conv_1",
              userId: "user123",
        ...
        // Only show error during initial load, not during background polling
        if (!preserveReadStatus) {
          setError(`Getting things ready... (Attempt ${fetchAttemptsRef.current})`)
        }

        // Shorter retry delay (max 5 seconds)
        const retryDelay = Math.min(1000 * Math.pow(1.5, fetchAttemptsRef.current - 1), 5000)

        // Only retry if not exceeded max attempts and component is still mounted
  ...
  const attemptReconnection = () => {
    ...
    if (isCurrentlyOnline && !isOnline) {
      // Browser reports online but our state is offline, update state
      setIsOnline(true)
    }
    ...
    // If we're still offline according to the browser, show a message
  ...
  return (
    <>
      ...
      {!isOnline && <OfflineNotification onRetry={attemptReconnection} />}
