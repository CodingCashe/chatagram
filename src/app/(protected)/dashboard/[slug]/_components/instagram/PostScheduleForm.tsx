...
const PostScheduleForm: React.FC<PostScheduleFormProps> = ({ userId, selectedMedia, onMediaClear }) => {
  const [caption, setCaption] = useState("")
    ...
    try {
      const response = await fetch("/api/post-to-instagram", {
        ...
        body: JSON.stringify({
          ...
          caption,
          mediaUrls: selectedMedia.map((media) => media.url),
          mediaType: selectedMedia.length > 1 ? "CAROUSEL" : selectedMedia[0].type.toUpperCase(),
        }),
      })
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      />
      <Button type="submit" disabled={isPosting} className="w-full bg-blue-600 hover:bg-blue-700 text-white">
