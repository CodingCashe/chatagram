...
import Image from "next/image"
import { motion, AnimatePresence, useAnimation } from "framer-motion"
import { ChevronLeft, ChevronRight, Plus, X, Video, Shuffle, ZoomIn, ZoomOut, Edit, Grid, List } from "lucide-react"
import { Button } from "@/components/ui/button"
import { useInView } from "react-intersection-observer"
import { Slider } from "@/components/ui/slider"
import { Popover, PopoverContent, PopoverTrigger } from "@/components/ui/popover"
...
const MediaSelector: React.FC<MediaSelectorProps> = ({ onMediaSelect, selectedMedia }) => {
  ...
  const [overlayOpacity, setOverlayOpacity] = useState(0)
  const canvasRef = useRef<HTMLCanvasElement>(null)
  const [activeSection, setActiveSection] = useState<"browse" | "selected" | "edit">("browse")
  ...
  const renderBrowseSection = () => (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <Button variant="outline" size="sm" onClick={() => setIsGridView(!isGridView)}>
          {isGridView ? <List className="mr-2 h-4 w-4" /> : <Grid className="mr-2 h-4 w-4" />}
          ...
          {selectedMedia.map((media) => (
            ...
            >
              <div className="w-full h-full rounded-md overflow-hidden">
                {media.type === "image" ? (
                  <Image src={media.url || "/placeholder.svg"} alt={media.name} layout="fill" objectFit="cover" />
                ) : (
                  <video src={media.url} className="w-full h-full object-cover" loop muted playsInline />
                )}
              </div>
      ...
      {editingMedia && (
        <div className="space-y-4">
          <div className="aspect-video relative overflow-hidden rounded-lg">
            <Image
              src={editingMedia.url || "/placeholder.svg"}
          ...
          <div>
            <label className="text-sm text-gray-300">Brightness</label>
            <Slider value={[brightness]} min={0} max={200} step={1} onValueChange={([value]) => setBrightness(value)} />
          </div>
          <div>
            <label className="text-sm text-gray-300">Contrast</label>
            <Slider value={[contrast]} min={0} max={200} step={1} onValueChange={([value]) => setContrast(value)} />
          ...
          <div>
            <label className="text-sm text-gray-300">Saturation</label>
            <Slider value={[saturation]} min={0} max={200} step={1} onValueChange={([value]) => setSaturation(value)} />
          </div>
          <div>
            <label className="text-sm text-gray-300">Overlay Color</label>
            <Popover>
  ...
  return (
    ...
    >
      ...
      <div className="mb-4">
        <Select
          value={activeSection}
          onValueChange={(value: "browse" | "selected" | "edit") => setActiveSection(value)}
        >
          ...
          <SelectContent className="bg-gray-700 text-gray-200 border-gray-600">
            <SelectItem value="browse">Browse Media</SelectItem>
            <SelectItem value="selected">Selected Media</SelectItem>
            <SelectItem value="edit">Edit Media</SelectItem>
          </SelectContent>
        </Select>
      </div>
      <Card className="bg-gray-700 border-gray-600">
        <CardContent className="p-4">
          <ScrollArea className="h-[calc(100vh-16rem)]">
            ...
            {activeSection === "selected" && renderSelectedSection()}
