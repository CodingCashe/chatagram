...
import { cn } from "@/lib/utils"
import { validateMediaForInstagram, validateCarouselMedia } from "@/lib/validate-media"
import { HashtagAnalyzer } from "./hashtag-analyzer"
import { TimeOptimizer } from "./time-optimizer"
import { ABTesting } from "./ab-testing"
import { MonetizationInsights } from "./monetization-insight"
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs"
import AIContentGenerator from "./ai-content-generator"
...
export function PostScheduler({ userId }: PostSchedulerProps) {
  ...
  const handleHashtagSuggestions = (hashtags: string[]) => {
    const newCaption = caption + " " + hashtags.map((tag) => `#${tag}`).join(" ")
  ...
  const handleCaptionVariant = (newCaption: string) => {
    setCaption(newCaption)
  }
        ...
        <Tabs defaultValue="upload" className="w-full">
          ...
          <TabsContent value="analytics" className="p-4 space-y-6">
            <div className="grid gap-6 md:grid-cols-2">
              <HashtagAnalyzer caption={caption} onSuggest={handleHashtagSuggestions} />
              <TimeOptimizer onSelectTime={handleTimeSelection} />
            </div>
            <ABTesting originalCaption={caption} onSelectVariant={handleCaptionVariant} />
