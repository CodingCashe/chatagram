...
async function retryOpenAIRequest<T>(requestFunction: () => Promise<T>, retries = MAX_RETRIES): Promise<T> {
  let lastError: Error | null = null
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      console.error(`OpenAI request failed (attempt ${i + 1}):`, error)
      lastError = error as Error
      await delay(INITIAL_RETRY_DELAY * Math.pow(2, i))
    }
  }
...
export async function POST(req: Request) {
  try {
    ...
    if (error instanceof Error) {
      if (error.message.includes("429")) {
        errorMessage = "OpenAI service is busy. Please wait a moment and try again."
      } else if (error.message.includes("No image generated")) {
        errorMessage = "Failed to generate image. Please try a different prompt."
      }
    }
