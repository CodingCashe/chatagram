...
const Page = async ({ searchParams: { code } }: Props) => {
  try {
    ...
    if (!code) {
      console.error('🔴 Error: No code parameter received.');
      return; // Stop execution since no code is provided
    }
    ...
    // Attempt to integrate the user using the cleaned code
    const user = await onIntegrate(cleanedCode);
    console.log('🟢 Integration response:', user);

    // Log whether the integration was successful
    if (user?.status === 200) {
      console.log('🟢 Integration successful:', user.data);
    } else {
      console.error(
        '🔴 Integration failed with status:',
        user?.status,
        'Response:',
        user
      );
